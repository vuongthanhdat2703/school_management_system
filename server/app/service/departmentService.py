...
class DepartmentService():
    ...
    def get_by_userID(self, id):
        deparment = self.session.query(Department).filter(
            Department.user_id == id).first()
        return deparment
    ...
    def get_name(self):
        ...
        for deparment in db:
            deparment_dict = {
                'id': deparment.id,
                'department_name': deparment.departments_name
            }
    ...
    def update_departments(self, id, lastName=None, firstName=None, email=None, phone=None, departments_name=None, start_date=None):
        update_department = self.session.query(
            Department).filter(Department.id == id).first()
        UserService(self.session).update_user(
            update_department.user_id, lastName, firstName, email, phone)
        if departments_name:
