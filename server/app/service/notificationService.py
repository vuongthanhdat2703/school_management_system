...
class NotificationService():
    ...
    def get_all_byID_Student(self, id_user):
        student = self.session.query(Student).filter(Student.id == id_user).first()
        manage = self.session.query(Students_Department).filter(Students_Department.student_id == student.id).all()
        for id in manage:
            get_notification = self.session.query(Notification).filter(Notification.department_id == id.department_id).all()
